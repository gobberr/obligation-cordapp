...
import com.example.flow.CreatePaperSell
import com.example.flow.ClosePaperSell
import com.example.flow.MakePurchase.Initiator
import net.corda.core.contracts.UniqueIdentifier

val SERVICE_NAMES = listOf("Notary", "Network Map Service", "Networkmap")
...
class AuctionApi(private val rpcOps: CordaRPCOps) {
    ...
    @Path("auctions")
    @Produces(MediaType.APPLICATION_JSON)
    fun getAuctions() = rpcOps.vaultQueryBy<Sell>().states

    /**
     * Get full Sell details.
     */
    @GET
    @Path("get-auction")
    @Produces(MediaType.APPLICATION_JSON)
    fun getAuction(@QueryParam("linearID") linearID: String): Response {
        if (linearID == "") {
            return Response.status(BAD_REQUEST).entity("Query parameter 'paperID' missing or has wrong format.\n").build()
        }
        ...
        return try {
            Response.ok(rpcOps.vaultQueryBy<Sell>(criteria=criteria).states).build()
        } catch (ex: Throwable) {
    ...
    fun createAuction(@QueryParam("itemName") itemName: String,@QueryParam("itemDescription") ItemDescription: String,@QueryParam("startPrice") startPrice: Int,@QueryParam("ExpiryDate") ExpiryDate: String,@QueryParam("AuctionParticipants") AuctionParticipants: String): Response {
        ...
        return try {
            val signedTx = rpcOps.startTrackedFlow(::CreatePaperSell, itemName, ItemDescription, startPrice, ExpiryDate, AuctionParticipants).returnValue.getOrThrow()
    ...
    /**
     * Initiates Make Purchase Flow.
     */
    @PUT
    @Path("make-bid")
    fun makeBid(@QueryParam("amount") amount: Int,@QueryParam("AuctionReference") AuctionReference: String): Response {
        if (AuctionReference == "" ) {
            return Response.status(BAD_REQUEST).entity("Query parameter 'Paper Reference' must not be null.\n").build()
        }
    ...
    /**
     * Initiates End Sell Flow.
     */
    @PUT
    @Path("end-auction")
    fun endAuction(@QueryParam("AuctionReference") AuctionReference: String): Response {
        return try {
            val signedTx = rpcOps.startTrackedFlow(::ClosePaperSell, AuctionReference).returnValue.getOrThrow()
            Response.status(CREATED).entity("Transaction id ${signedTx.id} committed to ledger.\n").build()
        } catch (ex: Throwable) {
            logger.error(ex.message, ex)
            Response.status(BAD_REQUEST).entity(ex.message!!).build()
